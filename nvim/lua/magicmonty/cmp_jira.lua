...
local function file_exists(file)
  if empty(file) == 1 then return false end
...
local function getCredentials()
  ...
  if not credentials
    or not credentials.username or empty(credentials.username) == 1
    or not credentials.password or empty(credentials.password) == 1
    or not credentials.url or empty(credentials.url) == 1 then
    local url
...
source.complete = function(self, params, callback)
  if not self.credentials
    ...
    or not self.credentials.url then return end
  ...
  if not cache or cache.isIncomplete then
    local url = self.credentials.url..'/rest/api/2/search?jql=project='..project..'%20AND%20statusCategory!="To%20Do"%20AND%20assignee%20was%20currentUser()%20ORDER%20BY%20key'
    ...
    Job:new({
      command = "curl",
      args = {
        "-u", self.credentials.username..":"..self.credentials.password,
        "-X", "GET",
      ...
      on_exit = function(job)
        ...
        self.cache[cache_key] = { items = items, isIncomplete = isIncomplete, startAt = parsed.startAt, maxResults = parsed.maxResults, total = parsed.total }
      end
    }):start()
  else
...
source.make_regex = function(_, option)
  ...
  if not option.slugs then return result end
...
source.get_keyword_pattern = function (_, params)
  local result =  "\\(\\u\\{3,}\\)-\\d*"
  if not params.option.slugs then return result end
...
source.is_available = function (self)
  if not self.credentials then return false end
